...
<body>
           	...
           	<span style="font-size:40px; color:#058AA8; font-family:Museo;"> User Feedbacks(All) </span><br>
				 <br><br>
<div id="demo"></div>
...
$gtx_query.=" WHERE date> DATE_SUB(NOW(), INTERVAL 6 MONTH) ORDER BY RAND()";
$gtx_result=mysql_query($gtx_query,$gtx_connection);

if(mysql_num_rows($gtx_result)==0 )
{ 
echo "There are no customer feedbacks since past six months.";
...
$time = date('d M g:i A ', strtotime($row['date']));//displaying current time using this
...
$(document).ready(function(){
    ...
    $(\"#{$trim5}\").click(function(){
        $(\"#{$trim6}\").show();
			...
			<span style=\"font-size:24px; font-family:Museo \">{$row['subject']} </span><br/> <span style=\"font-size:14px; font-family:Museo \">{$diff}</span>
